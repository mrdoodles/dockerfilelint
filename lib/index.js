...
module.exports.run = function (configPath, content) {
  ...
  for (var idx in instructions) {
    ...
    if (result.command === "cmd") {
      ...
      result.command === "from" &&
      state.stages[state.stages.length - 1].instructionsProcessed !== 0
    ) {
...
function loadRules(configPath) {
  ...
  var configFileContents = tryLoadFile(
    path.join(configPath, ".dockerfilelintrc")
  );
        ...
        apt.aptget_commands(commands).forEach(function (aptget_command, index) {
          ...
          if (["install", "remove", "upgrade"].indexOf(subcommand) > -1) {
            if (!apt.aptget_hasyes(aptget_command)) {
              items.push(
                messages.build(state.rules, "apt-get_missing_param", line)
              );
            }
          }
          ...
          if (subcommand === "install") {
            if (!apt.aptget_hasnorecommends(aptget_command)) {
              items.push(
                messages.build(state.rules, "apt-get_recommends", line)
              );
            }
          } else if (subcommand === "update") {
            if (!apt.follows_rmaptlists(commands, index)) {
              items.push(
                messages.build(state.rules, "apt-get_missing_rm", line)
              );
            }
            ...
            items.push(
              messages.build(state.rules, "apt-get-dist-upgrade", line)
            );
          }
        });
        if (
          aptgetSubcommands.indexOf("update") > -1 &&
          aptgetSubcommands.indexOf("install") === -1
        ) {
          items.push(
            messages.build(state.rules, "apt-get-update_require_install", line)
          );
        }
        ...
        apk.apk_commands(commands).forEach(function (apk_command, index) {
          ...
          if (subcommand === "add") {
            ...
            if (apk.apkadd_numpackages(apk_command) > 1) {
              if (num_apkdel > 0) {
                if (!apk.apkadd_hasvirtual(apk_command)) {
                  items.push(
                    messages.build(state.rules, "apkadd-missing-virtual", line)
                  );
                }
              }
            }
            if (!apk.apkadd_hasnocache(apk_command)) {
              if (
                !apk.apkadd_hasupdate(apk_command) ||
                !apk.follows_rmapkcache(commands, index)
              ) {
                items.push(
                  messages.build(
                    state.rules,
      ...
      case "expose":
        ...
        args.match(/\S+/g).forEach(function (port) {
          if (!checks.expose_port_valid(port)) {
            if (!port.includes(":")) {
              // Just eliminate a double message here
